...
**التقارير:** `GET /api/reports/daily-profit` صار يطرح خصومات اليوم من الإيراد، فيتطابق مع `totalSales` في `GET /api/invoices/daily-sales` — لكن فقط للفواتير المُنشأة عبر الـ API على سيرفر يعمل بتوقيت UTC، لأن `daily-sales` يحسب حدود اليوم بالتوقيت المحلي للسيرفر بينما `daily-profit` يحسبها بـ UTC (يتطابقان حالياً لأن حاوية الإنتاج نفسها تعمل بـ UTC). التطابق يفترض أيضاً أن مجموع بنود كل فاتورة يساوي `total + discount` بالضبط — وهذا غير مضمون لفواتير المزامنة الأوف‌لاين.

### توضيحات إضافية للفرونت

**⚠️ `discount: null` يُلغي الخصم، ولا يحافظ عليه.** الحفاظ على الخصم الحالي يحصل **فقط عند عدم إرسال الحقل `discount` إطلاقاً** ضمن جسم الـ `PATCH` — أي حذفه من الـ payload تماماً، لا إرساله بقيمة فارغة. السيرفر يتحقق من الشرط `dto.discount !== undefined`، و`null` قيمة **مُعرَّفة** (defined) تُحقّق هذا الشرط بنجاح، فتُعامَل كخصم جديد صريح، ثم تتحوّل داخلياً إلى `0` (تماماً كما لو أرسلت `discount: 0`). نفس الأمر ينطبق على الـ string الفارغة `discount: ""` — تتحوّل رقمياً إلى `0` أيضاً.

بكلام تاني: أي حقل رقمي منه في الفورم بيتفرّغ ويتحوّل لـ `null` أو `""` عند الإرسال (بدل ما يُحذف من الـ body بالكامل) هيصفّر الخصم فعلياً ويرفع صافي الفاتورة (`total`) للإجمالي الكامل مرة تانية — وده على الأغلب مش القصد لو المستخدم كان بس بيمسح الخانة مؤقتاً بدون نية الإلغاء.

**التصميم المطلوب في الفرونت:**
- لو خانة الخصم فاضية ولم يلمسها المستخدم أصلاً (أو رجّعها فاضية بدون قصد الإلغاء) → **لا** تُضمّن `discount` في جسم الطلب إطلاقاً؛ اتركها محذوفة بالكامل من الـ object المُرسَل.
- لو المستخدم يقصد إلغاء الخصم فعلاً → ابعت `discount: 0` بشكل صريح (سلوك مقصود وواضح، لا فرق عملي بينه وبين إرسال `null`).

**نقطة إضافية:** `discount` موجود أيضاً في استجابة تفاصيل الدين `GET /api/debts/:id`، داخل كائن `invoice`، بجانب `total`:
```json
{
  "invoice": {
    "id": "uuid",
...
- الطلب بيقبل `clientOperationId` اختياري (نص، حد أقصى ٢٠٠ حرف). إعادة نفس
  ...
  بنفس المُعرّف **لنفس الزبون: الاتنين بيرجّعوا `201`**، والمبلغ بينطبق مرة
  وحدة. الطلب التاني بيستنى على قفل صف الزبون وبعدين بيقرا العملية المكتوبة
  فبيرجّعها كإعادة نظيفة — **ما في `409` بهالحالة، فلا تبنوا فرع للتعامل معه.**
- **استخدام نفس `clientOperationId` لعميل مختلف عن اللي استُخدم فيه أول مرة
  يرجّع `409 Conflict`** (رسالة الخادم: `مُعرّف العملية مستخدم لعميل آخر`) —
  مش `400`. العملية الأصلية ما بتتلمس بهالحالة.
- الدفع الزائد صار **مقبول**. الفائض بينحفظ كرصيد للزبون.
- زبون ما عليه ديون صار يقدر يدفع — كامل المبلغ بيصير رصيد.
- **`amount` إله سقف أقصى `99999999.99`** (نفس دقة أعمدة `DECIMAL(10,2)`
  المخزَّن فيها). تجاوزه يرجّع `400` برسالة: `المبلغ يتجاوز الحد الأقصى
  المسموح به (99999999.99)`.
- **سقف تاني منفصل ومهم: تراكم رصيد العميل نفسه (`creditBalance`) ما بيتجاوز
  نفس الحد `99999999.99`.** حتى لو كل دفعة لحالها تحت السقف الأول، دفعتان
  شرعيتان على التوالي ممكن يوصّلوا الرصيد التراكمي فوق الحد الأقصى. بهالحالة
  الخادم يرفض العملية بـ `400` برسالة: `رصيد العميل بعد هذه العملية يتجاوز
  الحد الأقصى المسموح به (99999999.99)`. الفرونت لازم يتعامل مع هالخطأ زي أي
  400 عادي (مش استثناء خاص)، بس يعرف إنه ممكن يصير حتى لو كل دفعة فردية شكلها
  سليمة.
...
- كل صف في `affectedDebts` صار عنده `creditPaid` جنب `amountPaid`.
  **`amountPaid` هو الكاش فقط** (نفس معناه اليوم)، والرصيد المستهلَك في
  ...
  `Σ creditPaid = creditApplied`. **ملاحظة: `Σ amountPaid = paymentApplied`
  وحدها مش صحيحة عند وجود فائض** — لازم تجمع `excessToCredit` معاها.
...
نقطة تقنية إضافية تخص هالفيتشر تحديداً: `JSON.stringify` بيطلع نفس ناتج
`toString()` بالضبط (تحقّقنا: `new Decimal('50.10').toString()` و
`JSON.stringify(new Decimal('50.10'))` الاتنين يطلعوا `"50.1"` — الصفر
اللاحق بيروح). يعني **عمود مخزَّن مباشرة وناتج جمع محسوب في نفس الطلب
بيتسلسلوا بنفس الشكل تماماً** — ما فيش فرق بصيغة السلسلة تقدروا تستخدموه
للتفريق بين "قيمة من الداتابيز" و"قيمة محسوبة على الطاير". لا تفترضوا عدد
منازل عشرية ثابت حتى لو شفتوه ثابت في أمثلة الاختبار.
...
2. **بيعة دين أوف‌لاين ما بتأكل رصيد** وقت الرفع عبر `POST /sync/push`. هذا
   **سلوك متوقَّع ومقصود، مش خلل** — مسار الـ push ما بيلمس جدول الرصيد
   إطلاقاً بتصميمه. الرصيد بيضل محفوظ وبيتقاصّ بأول دفعة أو فاتورة أونلاين
   بعدها. الرقم الموقَّع (`balance`) صحيح بكل الأحوال لأنه محسوب من الطرفين.
3. **`POST /debts/:id/pay` (الدين المفرد) ما بيقبل دفع زائد** وما بيدعم
   `clientOperationId` (مؤكَّد من الكود — الحقل غير موجود إطلاقاً بالـ DTO).
   دفع أكبر من المتبقي يرجّع `400`. خارج النطاق عمداً.
4. **حذف دفعة كاش ولّدت رصيداً انصرف بيرجع `400`** برسالة
   `لا يمكن حذف هذه الدفعة — الرصيد الناتج عنها تم استخدامه`.
5. **أرشفة زبون معه رصيد بترجع `400`** برسالة: `لا يمكن أرشفة العميل — لديه
   رصيد لم يُستخدم بعد. اصرف الرصيد أو احذف الدفعة التي ولّدته أولاً.`
6. حذف أو تعديل فاتورة أكلت رصيداً بيرجّع الرصيد للزبون تلقائياً.
7. **رجوع رصيد لزبون مؤرشَف (isDeleted) بيرجّع أرشفته أوتوماتيك.** لو زبون
   اتأرشف بعدما وصل رصيده صفر، وبعدين انحذفت أو اتعدّلت دفعة/فاتورة قديمة
   وحرّكت رصيد راجع له، الخادم بيلغي أرشفته بنفس العملية (`isDeleted` يرجع
   `false`) — عشان المبلغ يضل ظاهر للمحل وما يختفيش من `/api/customers` و
   `/api/sync/init` وهو مبلغ لسه مدين فيه المحل للزبون. الفرونت ممكن يشوف
   زبون كان مؤرشَف يظهر تاني من غير أي طلب PATCH صريح لإعادة تفعيله.
